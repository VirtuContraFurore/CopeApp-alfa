...
public class SurveyDAO {
	...
	public static Survey getSurveyById (int surveyId) {
		try {
			TypedQuery<Survey> query = EntityManagerGlobal.getInstance().getEntityManager().createQuery("FROM surveys s WHERE (s.surveyId = :surveyId)", Survey.class);
	...
	public static void surveyCreate(User currentUser, Survey survey) {
		...
		if (canAccess) {
			EntityManagerGlobal.getInstance().getEntityManager().persist(survey);
	...
	public static ArrayList<SurveyMiniDTO> getSurveyMiniDTO(User currentUser, int lastSurveyNumber, int numberToRetrieve, boolean isMine, String filterKey, boolean isActive) {
		...
		if (!isMine) {
			...
			if (keyword.isEmpty()) {
				query = EntityManagerGlobal.getInstance().getEntityManager().createQuery("FROM Survey s JOIN FETCH s.answers a WHERE (s.closeSurveyDate "+active+" current_timestamp) AND (s.openSurveyDate > current_timestamp) ORDER BY s.closeSurveyDate DESC", Survey.class);
			} else {
				query = EntityManagerGlobal.getInstance().getEntityManager().createQuery("FROM Survey s JOIN FETCH s.answers a WHERE (s.closeSurveyDate "+active+" current_timestamp) AND (s.openSurveyDate > current_timestamp) LIKE :keyword ORDER BY s.closeSurveyDate DESC", Survey.class);
				query.setParameter("keyword", keyword);
			}
			query.setFirstResult(lastSurveyNumber);
			query.setMaxResults(numberToRetrieve);
		} else {
			query = EntityManagerGlobal.getInstance().getEntityManager().createQuery("FROM Survey s JOIN FETCH s.answers a WHERE (s.insertUser.userId = :userId) ORDER BY s.closeSurveyDate DESC", Survey.class);
	...
	public static void voteSurvey(User currentUser, int surveyId, List<AnswerDTO> answers) {
		...
		TypedQuery<Survey> query = EntityManagerGlobal.getInstance().getEntityManager().createQuery("SELECT Survey FROM Survey s WHERE (s.surveyId = :surveyId) order by date(s.closeSurveyDate) DESC ", Survey.class);
		query.setParameter("surveyId", surveyId);
		try {
			...
			if (!commonRole.isEmpty()) {
				for (AnswerDTO a: answers) {
					...
					EntityManagerGlobal.getInstance().getEntityManager().persist(v);
				}
				EntityManagerGlobal.getInstance().commitEntityManager();
			} else {
				throw new SurveyExcption(HttpStatusUtility.UNAUTHORIZED, MessageUtility.UNAUTHORIZED);
